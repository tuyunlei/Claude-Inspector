...
// --- Type Definitions (Moved here to decouple from UI) ---
...
export interface TimelineAction {
  ...
  payload?: any; // Input args (for tools) or Output content (for results/snapshots)
  isError?: boolean;
}
...
export interface ProjectTurn {
  id: string;
  timestamp: string; // Formatted start time
  queryNumber?: number;
  userQuery: string;
  replyPreview: string; // Claude's main text response
  ...
  // New Fields for merged system info
  guardrails?: string[]; // E.g. Caveat messages
  systemNotes?: string[]; // E.g. Compacted status messages
}

// [DEBUG] Track logged projects to prevent console spam
const loggedDebugProjects = new Set<string>();

/**
 * ANSI Strip Helper
 * Removes standard ANSI escape codes (e.g. [2m, [22m, colors)
 */
...
/**
 * Helper to extract raw text from mixed content blocks.
 */
...
function getActionKind(name: string): TimelineActionKind {
    if (!name) return 'tool';
